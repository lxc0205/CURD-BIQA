...
class Kadid_10kFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        ...
        for i, item in enumerate(index):
            for aug in range(patch_num):
                sample.append((os.path.join(root, 'distorted_images', imgname[item]), mos_all[item]))
...
class PIPALFolder(data.Dataset):
    ...
    def __init__(self, root, index, transform, patch_num):
        imgname = []
        mos_all = []
        label_path = os.path.join(root, 'Train', 'Label')
        # label_path 中有多个文件，需要遍历
        for file in os.listdir(label_path):
            if file.endswith('.txt'):
                with open(os.path.join(label_path, file), 'r') as f:
                    lines = f.readlines()
                    for line in lines:
                        # 处理每一行数据 A0001_00_00.bmp,1520.0648
                        parts = line.strip().split(',')
                        imgname.append(parts[0])
                        mos = np.array(float(parts[1])).astype(np.float32)
                        mos_all.append(mos)

        sample = []
        for i, item in enumerate(index):
            for aug in range(patch_num):
                sample.append((os.path.join(root, 'Train', 'Distortion', imgname[item]), mos_all[item]))
...
# need to test
class AGIQA3KFolder(data.Dataset):
    def __init__(self, root, index, transform, patch_num):
